...
df_full <- df_sdoh_sci_fr[8:45] - df_sdoh_sci_user[ 8:45]
df_full$distance <- df_s$distance$distance
df_full$probabilities <- df_s$probabilites
...
lm.fit <- lm(scaled_sci~., data = df_full[2:41])
summary(lm.fit)
```
```{r}
df_full$log_scaled_sci <- log(df_full$scaled_sci)
plot(df_full$log_scaled_sci ,df_full$ACS_TOT_POP_US_ABOVE1_ZC)
```
