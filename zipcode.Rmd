...
df_s$Count <- (1:261227)
df_sdoh_sci_fr <- merge(df_s, df_sdoh_pa_selected, by.x = 'fr_loc', by.y = 'ZIPCODE') %>% sort(by = Count)
df_sdoh_sci_user <- merge(df_s, df_sdoh_pa_selected, by.x = 'user_loc', by.y = 'ZIPCODE') %>% sort(by = Count)
df_<- df_sdoh_sci_fr[7:44] - df_sdoh_sci_user[ 7:44]
```
