...
***Document variables meta data, mobility based sci- criterion**
```{r}
...
MSE_Name_Same <- c('Full MLR Same Zip')
```
...
MSE_Name_Same <- c(MSE_Name_Same, 'Fwd Select Same')
```
...
MSE_Name_Same <- c(MSE_Name_Same, 'Ridge Same')
```
...
#Add MSE and Name to lists
MSE_All_Same <- c(MSE_All_Same, MSE.lasso)
