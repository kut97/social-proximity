...
```
The MSE with Lasso is slightly lower than the MSE we got from forward selection: 1.068451 vs 1.06849.

Next we will create a tree
```{r, warning = FALSE}
set.seed(77)
#Build and plot the tree
tree <- tree( scaled_sci ~ ., data, subset = train_index )
plot(tree)
summary(tree)
...
MSE.tree <- mean((tree.pred - test$scaled_sci)^2)
MSE.tree
...
#Add MSE and Name to lists
MSE_All <- c(MSE_All, MSE.ridge_same)
MSE_Name <- c(MSE_Name, 'Ridge Same')
```
...
MSE_Name <- c(MSE_Name, 'Full Tree')
```
The MSE is higher for the decison tree than it is for Lasso or forward selection.