...
We create the social proximity values for each county in Penselvaniya as a first step we want to convert the SCI values into relative probabilities so we divide the SCI by the highest frequency of SCI and create a data frame with user location fr_locations and there probability values.
...
df_1 <- unique(df_1)
df_1 <- df_1 %>% distinct(probabilites,.keep_all = TRUE) ##single repeted pair wise comparisions for sci
user_loc <- df_1 %>% distinct(user_loc)
```

Generating weights for 67 counties and merging it in a dataframe.
```{r}
...
Further to create a weighted probabilities scaled by SCI and the weights of friend location we create a new dataframe and mutate it with weighted SCI. 
```{r}
...
First create an adjancy matrix between user_loc and friendship_loc with probabilites as there enteries. Further we make all the diagonal matrix 0. 
